...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    // MARK: Method
    private func store() {
        // ลบข้อมุลที่เก็บไว้ทั้งหมด
        let domain = Bundle.main.bundleIdentifier!
        UserDefaults.standard.removePersistentDomain(forName: domain)
        UserDefaults.standard.synchronize()
        
        // เก็บข้อมูลใหม่
        UserDefaults.standard.set(self.todoList.count, forKey: "count")
        for index: Int in 0..<self.todoList.count {
            let todo = self.todoList[index]
            UserDefaults.standard.set(todo.message, forKey: "message\(index)")
            UserDefaults.standard.set(todo.completed, forKey: "completed\(index)")
        }
    }
    ...
    private func load() {
        ...
        for index: Int in 0..<count {
            let message = UserDefaults.standard.string(forKey: "message\(index)")!
            let todo: TodoItem = TodoItem(withMessage: message)
            todo.completed = UserDefaults.standard.bool(forKey: "completed\(index)")
